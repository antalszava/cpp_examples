...
int main(int argc, char *argv[])
{
  ...
  std::cout << "using_dropout = " << using_dropout << std::endl;
    ...
    std::vector<std::vector<double>> relu_deriv_layer_1(batch_size);
    ...
    for (int l = 0; l < batch_size; l++){
      ...
      relu_deriv_layer_1[l].resize(hidden_layer_size);
    }
	...
	for (int l= 0; l < batch_size; l++){
	  for (int j = 0; j < hidden_layer_size; j++)
	    layer_1[l][j] = relu(layer_1[l][j]/255.);
	}
	...
	for (int l= 0; l < batch_size; l++){
	  for (int j = 0; j < hidden_layer_size; j++){	
	    relu_deriv_layer_1[l][l] = relu_deriv(layer_1[l][l]);
	    if (using_dropout){
	      relu_deriv_layer_1[l][l] *= dropout_mask[l][l];
	    }
	  }
	}

	for (int i = 0; i < batch_size; i++){
	  layer_2[i] = matrix_multiplication(layer_1[i],weights_1_2);
	  for (int l = 0; l < num_labels; l++){
	    layer_2_delta[i][l] = -(layer_2[i][l] - training_1hot_labels[batch*batch_size + i][l]);


	    error += layer_2_delta[i][l]*layer_2_delta[i][l];
	    layer_2_delta[i][l] /= (double)batch_size;
	  }
	...
	for (int i = 0; i < batch_size; i++){
	  ...
	  layer_1_delta[i] = vector_component_mult(matrix_multiplication(layer_2_delta[i], weights_1_2, 1 /*tranpose*/), relu_deriv_layer_1[i] /*really relu_deriv(layer_1)*/);
	  for (int l = 0; l < num_labels; l++){
	    for(int j = 0; j < hidden_layer_size; j++){
